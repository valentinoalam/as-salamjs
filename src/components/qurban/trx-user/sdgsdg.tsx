...
import { ChevronLeft, ChevronRight, Check, Calendar, User, CreditCard, Package } from "lucide-react"
...
import { userQurbanSchema, type UserQurbanFormValues, type TipeHewan, type ProdukHewan } from "@/types/qurban-form"
...
const STEPS: Step[] = [
  ...
  {
    id: "products",
    title: "Pilih Jatah Mudhohi",
    description: "Pilih produk yang ingin diambil",
    icon: Package,
  },
  {
    id: "personal",
...
export function UserQurbanForm({ tipeHewan, produkHewan, onSubmit, onCancel, initialData }: UserQurbanFormProps) {
  ...
  const watchedValues = watch(["tipeHewanId", "quantity", "isKolektif", "mengambilDaging"])
  ...
  const handleSubmitForm = async (data: UserQurbanFormValues) => {
    ...
    try {
      ...
      if (result.success) {
        toast({
          title: "Berhasil!",
          description: "Pemesanan qurban berhasil dibuat. Silakan lakukan pembayaran sesuai instruksi yang diberikan.",
        })
      } else {
  ...
  const getProductSelectionConfig = () => {
    const [tipeHewanId, isKolektif, mengambilDaging] = watchedValues
    const animal = tipeHewan.find((t) => t.id.toString() === tipeHewanId)

    if (!animal || !mengambilDaging) {
      return null
    }

    const animalType = animal.jenis.toLowerCase() as "sapi" | "domba" | "kambing"

    if (animalType === "sapi") {
      if (isKolektif) {
        return {
          maxSelection: 1,
          allowedCategories: ["DAGING"],
          animalType,
          isKolektif: true,
          description: "Pilih 1 jenis daging (5kg per mudhohi)",
        }
      } else {
        return {
          maxSelection: 2,
          allowedCategories: ["DAGING", "JEROAN", "TULANG", "KULIT"],
          animalType,
          isKolektif: false,
          description: "Pilih maksimal 2 produk untuk dibawa pulang",
        }
      }
    } else if (animalType === "domba" || animalType === "kambing") {
      return {
        maxSelection: 1,
        allowedCategories: ["DAGING"],
        animalType,
        isKolektif: false,
        description: "Anda akan mendapatkan kaki belakang",
        disabled: true, // Cannot choose for lamb/goat
      }
    }
  ...
  const productConfig = getProductSelectionConfig()
            ...
            <CardContent className="space-y-6">
              ...
              {currentStep === 1 && (
                <div className="space-y-6">
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                        ...
                        <div className="space-y-1 leading-none">
                          <FormLabel>Mengambil Jatah Mudhohi</FormLabel>
                  ...
                  {form.watch("mengambilDaging") && productConfig && (
                    <div className="space-y-4">
                      <div className="p-4 bg-blue-50 rounded-lg">
                        <h4 className="font-medium text-blue-900 mb-2">Informasi Jatah Mudhohi</h4>
                        <p className="text-blue-800 text-sm">{productConfig.description}</p>
                      </div>
                          ...
                          <FormItem>
                            ...
                            <FormControl>
                              <ProductCardSelection
                                ...
                                maxSelection={productConfig.maxSelection}
                                allowedCategories={productConfig.allowedCategories}
                                animalType={productConfig.animalType}
                                isKolektif={productConfig.isKolektif}
                                disabled={productConfig.disabled}
                              />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                  ...
                  {!form.watch("mengambilDaging") && (
                    <div className="text-center py-8 text-gray-500">
                      <Package className="h-12 w-12 mx-auto mb-4 text-gray-400" />
                      <p>Centang "Mengambil Jatah Mudhohi" untuk memilih produk</p>
                    </div>
                  )}
                </div>
              )}
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-muted p-6 rounded-lg space-y-4">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      {form.watch("mengambilDaging") && form.watch("selectedProductIds")?.length > 0 && (
                        <div className="flex justify-between">
                          <span>Jatah Mudhohi:</span>
                          <span className="font-medium">{form.watch("selectedProductIds")?.length} produk</span>
                        </div>
                      )}
                      <Separator />

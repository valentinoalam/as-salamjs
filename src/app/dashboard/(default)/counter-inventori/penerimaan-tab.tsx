...
const PenerimaanContentTab = () => {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-50 dark:bg-gray-900 p-4">
      ...
      <DialogContent className="max-w-md lg:max-w-4xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto my-4 pr-2">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {selectedShipment?.daftarProdukHewan.map((item) => {
              ...
              // Function to get icon and color based on product name
              const getProductStyle = (productName: string) => {
                const name = productName.toLowerCase()
                if (name.includes('kulit') || name.includes('leather')) {
                  return { icon: '🟫', bgColor: 'bg-orange-100', borderColor: 'border-orange-300' }
                } else if (name.includes('kepala') || name.includes('head')) {
                  if(name.includes('sapi')) return { icon: '🐮', bgColor: 'bg-blue-100', borderColor: 'border-blue-300' }
                  else return { icon: '🐴', bgColor: 'bg-blue-100', borderColor: 'border-blue-300' }
                } else if (name.includes('kaki') || name.includes('leg') || name.includes('foot')) {
                  return { icon: '🦵🏼', bgColor: 'bg-green-100', borderColor: 'border-green-300' }
                } else if (name.includes('ekor') || name.includes('tail')) {
                  return { icon: '⤵', bgColor: 'bg-purple-100', borderColor: 'border-purple-300' }
                } else if (name.includes('hati') || name.includes('liver')) {
                  return { icon: '❤️', bgColor: 'bg-red-100', borderColor: 'border-red-300' }
                } else if (name.includes('usus') || name.includes('intestine')) {
                  return { icon: '〰️', bgColor: 'bg-yellow-100', borderColor: 'border-yellow-300' }
                } else if (name.includes('daging') || name.includes('meat')) {
                  return { icon: '🥩', bgColor: 'bg-pink-100', borderColor: 'border-pink-300' }
                } else if (name.includes('tulang') || name.includes('bone')) {
                  return { icon: '🦴', bgColor: 'bg-gray-100', borderColor: 'border-gray-300' }
                } else {
                  return { icon: '📦', bgColor: 'bg-slate-100', borderColor: 'border-slate-300' }
                }
              }

              const productStyle = getProductStyle(item.produk.nama)
              
              return (
                <div 
                  key={item.id} 
                  className={`space-y-2 p-4 rounded-lg border-2 ${productStyle.bgColor} ${productStyle.borderColor}`}
                >
                  <div className="flex items-center gap-2">
                    <span className="text-2xl">{productStyle.icon}</span>
                    <Label htmlFor={`received-${item.produkId}`} className="font-medium">
                      {item.produk.nama}
                    </Label>
                  </div>
                  <div className="flex items-center gap-2">

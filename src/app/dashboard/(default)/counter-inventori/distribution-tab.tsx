...
type NewRecipientData = {
  ...
  jenisId?: string;
  keterangan?: string;
  jumlahKupon?: number; // Add this if you want to include kupon functionality
};
...
const DistributionContentTab = () => {
  ...
  const transactionHistory = useMemo(() => {
    ...
    return penerimaQuery.data
      ...
      .map(p => ({
        ...
        penerima: p.nama,
        jenis: p.jenis,
        produk: p.logDistribusi?.listProduk.map((lp: ProdukDiterima) => ({
          nama: lp.jenisProduk.nama,
          jumlah: lp.jumlahPaket,
        })),
        waktu: new Date(p.logDistribusi?.createdAt || "").toLocaleString("id-ID"),
        status: p.sudahMenerima ? "Selesai" : "Belum",
      }));
  }, [penerimaQuery.data]);
  ...
  const [newRecipientData, setNewRecipientData] = useState<NewRecipientData>({
    nama: '',
    diterimaOleh: '',
                ...
                {transactionHistory.map(transaction => (
                  <div key={transaction.id} className="border rounded-lg p-4">
                    ...
                    <div className="text-sm text-gray-600">
                      <strong>Produk:</strong> {transaction?.produk!.map((p: { nama: string; jumlah: number; }) => `${p.nama} (${p.jumlah})`).join(', ')}
                    </div>
                  </div>
                ))}
              </div>

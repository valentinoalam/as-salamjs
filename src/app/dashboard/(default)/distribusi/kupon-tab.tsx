...
import { StatusKupon, type Kupon } from '#@/types/qurban.ts';

const PengembalianKuponTab = () => {
  const [kuponList, setKuponList] = useState<Kupon[]>([]);
  ...
  useEffect(() => {
    const fetchKuponData = async () => {
      try {
        const response = await fetch('/api/kupon'); // Panggil API route Anda
        if (!response.ok) {
          // Tangani jika respons bukan 2xx (misalnya 404, 500)
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const result = await response.json();

        if (result.success) {
          // Pastikan data yang diterima sesuai dengan interface Kupon
          // Anda mungkin perlu memetakan atau memformat data di sini
          // jika struktur dari API tidak persis sama dengan interface Kupon.
          // Contoh pemetaan sederhana jika API hanya mengembalikan id dan status:
          const formattedData: Kupon[] = result.data.map((kupon: any) => ({
            id: kupon.id,
            status: kupon.status,
            penerima: kupon.penerima || 'N/A', // Default jika tidak ada dari API
            tanggalSalur: kupon.tanggalSalur || 'N/A', // Default jika tidak ada dari API
            mudhohi: kupon.mudhohi || 'N/A', // Default jika tidak ada dari API
          }));
          setKuponList(formattedData);
        } else {
          setError(result.message || 'Gagal memuat data kupon.');
        }
      } catch (err: any) {
        console.error('Error fetching kupon data:', err);
        setError(`Gagal memuat kupon: ${err.message}`);
      } finally {
        setLoading(false); // Selesai loading, baik berhasil atau gagal
      }
    };
  ...
  if (loading) {
    return <div className="text-center py-4">Memuat data kupon...</div>;
  }
  ...
  const filteredKupon = kuponList.filter(kupon => 
    (kupon.status === 'DISTRIBUTED' || kupon.status === 'RETURNED') &&
  ...
  const handlePengembalian = () => {
    if (selectedKupon.size === 0) {
      alert('Pilih minimal satu kupon untuk diRETURNEDkan');
      return;
    }

    const updatedKupon = kuponList.map(kupon => {
      if (selectedKupon.has(kupon.id)) {
        return { ...kupon, status: StatusKupon.RETURNED, updatedAt: new Date() } as Kupon;
      }
      return kupon;
    });
    ...
    setSelectedKupon(new Set());
    alert(`${selectedKupon.size} kupon berhasil diRETURNEDkan`);
  };
                ...
                filteredKupon.map((kupon) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {kupon.status === "RETURNED" && kupon.updatedAt?.toISOString().split('T')[1]}
                    </td>
                  </tr>
                ))
              )}
        ...
        <div className="px-6 py-4 bg-gray-50 rounded-b-lg">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            <div className="text-sm text-gray-600">
              Menampilkan {filteredKupon.length} kupon dari total {kuponList.filter(k => k.status === 'DISTRIBUTED').length} kupon yang disalurkan
            </div>
            <div className="text-sm text-gray-600">
              {selectedKupon.size} kupon dipilih untuk diRETURNEDkan
            </div>
          </div>
        </div>
      </div>

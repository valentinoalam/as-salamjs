...
const JatahMudhohiTab = () => {
  ...
  const { tabs, setActiveTab, forms, updateFormField, resetForm } = useUIState()
    ...
    switch (type) {
      ...
      case "penerima":
        data = penerima.map((p: Penerima) => ({
          ...
          "No Kupon": p.kupon.map(kuponItem => kuponItem.id).join(', ') || "-",
      ...
      case "kupons":
        data = penerima
          .filter((p) => p.kupon.map(kuponItem => kuponItem.id))
          .map((p) => ({
            "No Kupon": p.kupon.map(kuponItem => kuponItem.id).join(', '),
            Penerima: p.diterimaOleh || p.nama || "-",
            Kategori: p.distribusi?.kategori || "-",
            Status: p.sudahMenerima ? "Sudah Dikembalikan" : "Belum Dikembalikan",
          }))
        filename = "kupon_data"
        break
      case "mudhohi":
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-2xl font-bold text-blue-600">
                  {mudhohiQuery.data?.filter((m) => m.hewan.some((h: any) => h.tipe?.jenis === "SAPI")).length ||
                    0}
                </p>
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-2xl font-bold text-gray-600">
                  {mudhohiQuery.data?.filter((m) => m.hewan.every((h) => h.receivedByMdhohi)).length || 0}
                </p>

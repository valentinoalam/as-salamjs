...
import { useDistribusi } from "@/contexts/distribusi-context"
    ...
    switch (type) {
      case "distribusi":
        ...
        data = distributions.map((dist: Distribusi) => ({
          ...
          "Dibuat Pada": new Date(dist.dibuatPada).toLocaleDateString("id-ID"),
        }))
      ...
      case "penerima":
        ...
        data = filteredPenerima.map((p: Penerima) => ({
          ...
          Telepon: p.telepon || "-",
          Alamat: p.alamat || "-",
          "Jumlah Kupon": p.kupon?.length || 0,
          Status: p.sudahMenerima ? "Sudah Menerima" : "Belum Menerima",
          "Dibuat Pada": new Date(p.dibuatPada).toLocaleDateString("id-ID"),
        }))
                        ...
                        {distributions.map((dist: Distribusi) => (
                          <TableRow key={dist.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <TableCell className="text-sm text-gray-600">
                              {new Date(dist.dibuatPada).toLocaleDateString("id-ID")}
                            </TableCell>
                            <TableCell>
                              <Button
                                size="sm"
                  ...
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <CardDescription className="text-emerald-50">
                        Kategori: {distributions.find((d) => d.id === selectedDistribusi)?.kategori}
                      </CardDescription>
                    </div>
                    <div className="flex gap-2">
                      <Button
                  ...
                  {filteredPenerima.length === 0 ? (
                    ...
                    <div className="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200">
                      <Table>
                        ...
                        <TableBody>
                          {filteredPenerima.map((item: Penerima) => (
                            <TableRow key={item.id} className="hover:bg-gray-50 transition-colors">
                              <TableCell className="font-medium">{item.nama || "-"}</TableCell>
                              <TableCell>{item.diterimaOleh || "-"}</TableCell>
                              <TableCell>
                              ...
                              <TableCell>
                                <div className="flex items-center gap-1">
                                  ...
                                  <span>{item.kupon?.length || 0}</span>
                                </div>
                              </TableCell>
                              <TableCell>{getStatusBadge(item.sudahMenerima)}</TableCell>
                              <TableCell>
                                <div className="flex gap-1">
                                  <DistribusiProdukDialog
                                    penerimaId={item.id}
                                    currentProducts={item.logDistribusi?.listProduk || []}
                                    onSuccess={() => penerimaQuery.refetch()}
                                  />
                                </div>
                              </TableCell>
